...
async function createItem(req, res) {
    try {
        const userId = req.user.id;
        const validation = validation_js_1.CreateItemSchema.safeParse(req.body);
...
async function updateItem(req, res) {
    try {
        ...
        const validation = validation_js_1.UpdateItemSchema.safeParse(req.body);
        if (!validation.success) {
            return res.status(400).json({
                success: false,
                errors: validation.error.format()
        ...
        // Verify ownership
        const check = await (0, db_js_1.query)('SELECT id FROM items WHERE id = $1 AND user_id = $2', [itemId, userId]);
        ...
        const sql = `
            ...
            RETURNING *, (expiry_date - CURRENT_DATE) AS days_until_expiry
        `;
        const result = await (0, db_js_1.query)(sql, values);
        return res.status(200).json({
            success: true,
            data: result.rows[0]
        });
...
async function deleteItem(req, res) {
    try {
        ...
        if (permanent) {
            await (0, db_js_1.query)('DELETE FROM items WHERE id = $1 AND user_id = $2', [itemId, userId]);
            return res.status(200).json({
                success: true,
            ...
            const result = await (0, db_js_1.query)("UPDATE items SET status = 'consumed' WHERE id = $1 AND user_id = $2 RETURNING *", [itemId, userId]);
            return res.status(200).json({
                success: true,

...
import { ItemCategory } from '../utils/validation.js';
...
`;
        ...
        const parsed = JSON.parse(rawText) as ExtractedReceiptItem[];
...
export async function generateRecipesWithGemini(
    expiringItems: { name: string; category: string; quantity: string; expiry_date: string }[],
    allPantryItems: { name: string; quantity: string }[] = []
...
`;
        ...
        const rawText = response.text || '';
        const recipes = JSON.parse(rawText) as GeneratedRecipe[];
        return recipes;
    } catch (error) {
...
function validateCategory(cat: string): ItemCategory {
    const valid: ItemCategory[] = ['produce', 'dairy', 'meat', 'bakery', 'pantry', 'other'];
    const lower = (cat || '').toLowerCase() as ItemCategory;
...
function getFallbackReceiptItems(): ExtractedReceiptItem[] {
    const date = new Date();
...
function getFallbackRecipes(expiringItems?: { name: string }[]): GeneratedRecipe[] {
